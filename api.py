...
app = Flask(__name__)
CORS(app)  # Enable CORS for local Flutter testing
    ...
    try:
        ...
        for item in items:
            if item.get('@type') == 'BroadcastEvent':
                ...
                if not channels and isinstance(item.get('broadcastChannel'), list) and len(item['broadcastChannel']) > 0 and isinstance(item['broadcastChannel'][0], dict):
                    channels = [item['broadcastChannel'][0].get('name', 'Unknown')]
                ...
                # Split teams if possible (e.g. "Team A - Team B")
                home_team = name
                away_team = ""
                if " - " in name:
                    parts = name.split(" - ", 1)
